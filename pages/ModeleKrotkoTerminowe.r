...
# Ścieżki do plików
fileToAnalize <- 'Dane/companyValues.csv'
fileWithIndexes <- 'Dane/companies_symbol.csv'

if (file.exists("Wyniki/Predykcje/KrotkoTerminowa.csv")) {
        file.remove("Wyniki/Predykcje/KrotkoTerminowa.csv")
        message("Existing file removed.")
    }
...
# Dodanie przedrostka  do symboli
indexes$symbol <- paste0(indexes$trademark, ":", indexes$symbol)
...
if ((weighted_effectiveness >= x && weighted_effectiveness <= y)) {
    ...
    write.table(weighted_means, 
                file = "Wyniki/Predykcje/KrotkoTerminowa.csv", 
                sep = ",", 
                row.names = FALSE, 
                col.names = !file.exists("Wyniki/Predykcje/KrotkoTerminowa.csv"), # Dodaj nagłówki tylko raz
                append = TRUE)
